 ...
 */
export function triggerScreenShake(intensity: number = 5, duration: number = 300): Promise<void> {
 ...
 */
export function animateHealthBar(element: HTMLElement, from: number, to: number, max: number): Promise<void> {
    return new Promise((resolve) => {
        if (!element) {
            resolve();
            return;
 ...
 * @returns {Promise} Resolves when animation completes
 */
export function showVictorySplash(): Promise<void> {
    return new Promise((resolve) => {
 ...
 */
export function animateImpact(x: number, y: number, color: string = '#ef4444', particleSystem?: ParticleSystem): void {
    // Create impact flash
 ...
 * @param {ParticleSystem} particleSystem - Particle system instance
 */
export function animateBlock(x: number, y: number, particleSystem?: ParticleSystem): void {
    ...
    // Particles
    if (particleSystem) {
        // cast to any to use burst which might have options mismatch if strict
        (particleSystem as any).burst(x, y, 20, {
            color: '#3b82f6',
            size: 3,
            speed: 2,
            decay: 0.02,
 ...
 */
export function pulseElement(element: HTMLElement, color: string = '#8b5cf6'): void {
    if (!element) return;
 ...
 */
export function flashDamageNumber(x: number, y: number, damage: number, color: string = '#ef4444'): void {
 ...
 * @param {ParticleSystem} particleSystem - Particle system instance
 */
export function enemyDefeatedExplosion(x: number, y: number, particleSystem?: ParticleSystem): void {
    if (particleSystem) {
