...
/**
 * Orchestrates rendering of game state into the UI.
 * Acts as a bridge between the core Game class and specialized UI renderers.
 */
export class RenderController {
    ...
    /**
     * Renders the hero's hand cards.
     */
    renderHand(): void {
        if (this.game.ui) {
            this.game.ui.renderHandCards(
                this.game.hero.hand,
                (index: number, card: any) => this.game.interactionController.handleCardClick(index, card),
                (index: number, card: any) => this.game.interactionController.handleCardRightClick(index, card)
            );
        }
    }

    /**
     * Renders the mana source (dice).
     */
    renderMana(): void {
        if (this.game.ui) {
            this.game.ui.renderManaSource(
                this.game.manaSource,
                (index: number, color: string) => this.game.interactionController.handleManaClick(index, color),
     ...
     */
    renderAchievements(category: string): void {
        ...
        // Filter and Render
        const allAchievements = Object.values(this.game.achievementManager.achievements as Record<string, any>);
    ...
    /**
     * Renders the statistics grid in the modal.
     */
    renderStatistics(category: string): void {
    ...
    /**
     * Updates the phase indicator text and hint.
     */
     ...
     */
    private _getExploreTitle(canExplore: boolean, hasPoints: boolean): string {
