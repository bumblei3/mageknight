...
export class HexGridRenderer {
    ...
    constructor(canvas: HTMLCanvasElement, logic: HexGridLogic) {
        this.canvas = canvas;
    ...
    render(hero: any = null, enemies: any[] = []) {
        this.animationFrame++;
        this.clear();
    ...
    drawHexText(q: number, r: number, text: string, options: any = {}) {
        const pos = this.axialToPixel(q, r);
    ...
    drawHexIcon(q: number, r: number, icon: string, offsetY: number = 0) {
        const pos = this.axialToPixel(q, r);
    ...
    drawEnemy(q: number, r: number, enemy: any) {
        const pos = this.axialToPixel(q, r);
        const radius = 18;
    ...
    drawTerrainTexture(pos: { x: number; y: number }, terrain: string) {
        this.ctx.save();
        this.ctx.beginPath();
        for (let i = 0; i < 6; i++) {
            const angle = 2 * Math.PI / 6 * i;
    ...
    drawWaterTexture(pos: { x: number; y: number }) {
    ...
    drawForestTexture(pos: { x: number; y: number }) {
    ...
    drawMountainTexture(pos: { x: number; y: number }) {
    ...
    drawDesertTexture(pos: { x: number; y: number }) {
    ...
    drawPlainsTexture(pos: { x: number; y: number }) {
