...
describe('Combo System', () => {
    ...
    it('Helper Functions: isMonoColor returns true for same colors', () => {
        expect(CombatCombos.isMonoColor(['red', 'red', 'red'])).toBe(true);
    });

    it('Helper Functions: isMonoColor returns false for mixed colors', () => {
        expect(CombatCombos.isMonoColor(['red', 'blue', 'red'])).toBe(false);
    });

    it('Helper Functions: hasAllColors returns true with 4 different colors', () => {
        expect(CombatCombos.hasAllColors(['red', 'blue', 'green', 'white'])).toBe(true);
    });

    it('Helper Functions: hasAllColors returns false with only 3 colors', () => {
        expect(CombatCombos.hasAllColors(['red', 'blue', 'green'])).toBe(false);
    });

    it('Helper Functions: getColorName returns German color names', () => {
        expect(CombatCombos.getColorName('red')).toBe('Rot');
        expect(CombatCombos.getColorName('blue')).toBe('Blau');
        expect(CombatCombos.getColorName('green')).toBe('Grün');
        expect(CombatCombos.getColorName('white')).toBe('Weiß');
    });
